...
export default function Benefits() {
  ...
  return (
    <section id="benefits" className="bg-chalk overflow-hidden" ref={ref}>
      ...
      <div className="bg-ink px-5 py-16">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className={`transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <span className="inline-block border border-sky/40 text-sky text-xs font-bold px-4 py-1.5 rounded-full mb-4 tracking-widest uppercase">
              Kenapa remit.go!
            </span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-chalk leading-tight">
              3 Alasan PMI HK<br />
              <span className="text-sky">Pilih Kami.</span>
            </h2>
          </div>
          <p
            className={`text-chalk/40 text-sm max-w-xs md:text-right transition-all duration-700 ${visible ? 'opacity-100' : 'opacity-0'}`}
        ...
        >
          ...
          <div className="w-full md:w-1/2 bg-chalk flex flex-col justify-center px-8 md:px-14 py-12 md:py-16">
            <div className="flex items-center gap-3 mb-5">
              <span className="font-extrabold text-4xl leading-none opacity-20" style={{ color: '#1a47d6' }}>{u.num}</span>
              <span
                className="text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full border"
                style={{ color: u.accent, borderColor: u.accent + '60', background: u.accent + '18' }}
              >
                {u.tag}
              </span>
            </div>
            ...
            <div
              ...
              style={{ background: u.accent + '20', color: u.accent === '#7ec8e3' ? '#0f6080' : '#16803d' }}
            >
      ...
      {/* CTA strip */}
      <div
        className={`bg-brand px-5 py-12 transition-all duration-700 ${visible ? 'opacity-100' : 'opacity-0'}`}
      ...
      >
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-white font-extrabold text-xl md:text-2xl leading-tight">
            ...
            <span className="text-sky"> giliran kamu.</span>
          </p>
          <div className="flex gap-3 flex-shrink-0">
            <a
              href="https://play.google.com/store/apps/details?id=com.brchk.remitgo&pcampaignid=web_share"
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-white text-brand font-bold px-6 py-3.5 rounded-full text-sm hover:bg-chalk hover:scale-105 transition-all"
            >
            ...
            <a
              ...
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-white/15 border border-white/25 text-white font-bold px-6 py-3.5 rounded-full text-sm hover:bg-white/20 transition-all"
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
