...
export default function SectionCta({ text, sub }: SectionCtaProps) {
  return (
    <div className="bg-brand py-8 px-5 overflow-hidden">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-5">
        <div className="text-center sm:text-left">
          <p className="text-white font-extrabold text-xl leading-tight">{text}</p>
          {sub && <p className="text-sky text-sm mt-1">{sub}</p>}
        </div>
        <div className="flex gap-2.5 flex-shrink-0">
          <a
            href="https://play.google.com/store/apps/details?id=com.brchk.remitgo&pcampaignid=web_share"
            target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 bg-white text-brand font-bold px-5 py-3 rounded-full text-xs hover:bg-chalk hover:scale-105 transition-all"
          >
          ...
          <a
            ...
            target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 bg-white/15 border border-white/25 text-white font-bold px-5 py-3 rounded-full text-xs hover:bg-white/20 transition-all"
          >
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
