...
export default function Hero() {
  ...
  return (
    <section className="relative bg-ink min-h-screen overflow-hidden flex flex-col">
      ...
      {/* ── Ticker tape — brand blue ── */}
      <div className="bg-brand overflow-hidden pt-[72px]">
        <div className="flex animate-marquee whitespace-nowrap py-2.5">
          {[...ticker, ...ticker].map((t, i) => (
            <span key={i} className="inline-flex items-center gap-4 text-white font-bold text-xs uppercase tracking-widest px-6">
              {t}
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-5 pt-12 pb-8 flex flex-col md:flex-row md:items-stretch gap-8">
        ...
        <div className="flex-1 flex flex-col justify-between animate-slide-left">
          <div>
            {/* Badge */}
            <div className="inline-flex items-center gap-2 border border-sky/30 text-sky text-xs font-semibold px-4 py-2 rounded-full mb-8">
            ...
            <h1 className="text-[clamp(3rem,8vw,6rem)] font-extrabold text-chalk leading-[0.9] tracking-tight mb-6">
              ...
              <span
                ...
                style={{ WebkitTextStroke: '2px #7ec8e3', color: 'transparent' }}
              >
                Harusnya
              </span>
              <br />
              <span className="text-sky">Nggak</span>{' '}
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              <a
                ...
                className="inline-flex items-center gap-2.5 bg-brand text-white font-bold px-6 py-3.5 rounded-full text-sm hover:bg-brand-dark hover:scale-105 active:scale-95 transition-all shadow-lg shadow-brand/30"
              >
              ...
              <a
                ...
                target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2.5 bg-white/10 border border-white/20 text-chalk font-bold px-6 py-3.5 rounded-full text-sm hover:bg-white/15 transition-all"
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div ref={statsRef} className="flex gap-8 border-t border-white/10 pt-6">
            {[
              ...
              >
                <p className="text-2xl font-extrabold text-sky">{s.val}</p>
        ...
        <div className="flex-shrink-0 w-full md:w-[400px] lg:w-[460px] animate-slide-right">
          <div className="relative h-full min-h-[420px] md:min-h-0 rounded-3xl overflow-hidden">
            ...
            <div className="absolute top-1/3 left-5 bg-ink/80 border border-sky/20 text-chalk/80 font-semibold text-xs px-3 py-2 rounded-xl -rotate-1 shadow-lg backdrop-blur-sm">
              Harus jauh ke agen...
            </div>
        ...
        <div className="max-w-6xl mx-auto px-5 py-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={p.num} className="flex items-start gap-4">
              <span className="text-sky font-extrabold text-2xl leading-none opacity-50 flex-shrink-0">{p.num}</span>
              <div>
