    ...
    <section id="promo" className="bg-chalk py-20 overflow-hidden" ref={ref}>
      <div className="max-w-5xl mx-auto px-5">
        <div className={`mb-12 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <span className="inline-block border border-brand/25 text-brand text-xs font-bold px-4 py-1.5 rounded-full mb-4 tracking-widest uppercase">
            Promo Bulan Ini
          </span>
          <h2 className="text-4xl md:text-5xl font-extrabold text-ink leading-tight">
            Penawaran Spesial<br />
            <span className="italic text-brand">Untuk Kamu.</span>
          </h2>
        </div>

        <div
          className={`relative overflow-hidden rounded-3xl border-2 border-brand/20 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          <div
          ...
          >
            ...
            <div className="flex-shrink-0 w-full sm:w-[260px] md:w-[300px]">
              <div className="relative w-full" style={{ aspectRatio: '4/5' }}>
                ...
                />
                <span className="absolute top-4 left-4 inline-block bg-brand text-white text-[11px] font-extrabold px-3 py-1.5 rounded-full uppercase tracking-wide shadow">
                  {promo.badge}
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 p-7 md:p-10 flex flex-col justify-between bg-chalk">
              <div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-8">
                  {[
                    ...
                    <div key={label} className="bg-brand/8 border border-brand/15 rounded-2xl px-3 py-3 text-center">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 bg-brand hover:bg-brand-dark text-white font-bold px-6 py-3.5 rounded-full text-sm hover:scale-105 transition-all w-fit shadow-md shadow-brand/25"
              >
                {promo.cta}
          ...
          <button
            onClick={prev}
            className="absolute left-3 bottom-4 sm:top-1/2 sm:bottom-auto sm:-translate-y-1/2 z-20 w-9 h-9 rounded-full bg-brand text-white flex items-center justify-center hover:bg-brand-dark transition-all shadow"
          >
            <ChevronLeft size={18} />
          </button>
          <button
            onClick={next}
            className="absolute right-3 bottom-4 sm:top-1/2 sm:bottom-auto sm:-translate-y-1/2 z-20 w-9 h-9 rounded-full bg-brand text-white flex items-center justify-center hover:bg-brand-dark transition-all shadow"
          >
          ...
          {promos.map((_, i) => (
            <button
              ...
              style={{
                ...
                height: 8,
                background: i === current ? '#1a47d6' : '#c5d0e8',
              }}
            />
          ))}
        </div>
